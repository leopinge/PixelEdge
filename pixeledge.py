...
class JanelaPrincipal:
    ...
    def __init__(self, janela_raiz):
        ...
        self.fator_amplificacao = tk.DoubleVar(value=4.0)
        self._botoes_filtro     = {}
    ...
    def _criar_cabecalho(self):
        ...
        tk.Label(logo, text="Pixel", font=("Segoe UI", 22, "bold"),
                 fg=TEXT, bg=PANEL).pack(side="left")
        tk.Label(logo, text="Edge", font=("Segoe UI", 22, "bold"),
    ...
    def _criar_area_principal(self):
        ...
        painel_esq = tk.Frame(area, bg=PANEL, width=230,
    ...
    def _criar_controles(self, painel):
        tk.Label(painel, text="IMAGEM", font=("Segoe UI", 8, "bold"),
                 fg=MUTED, bg=PANEL).pack(anchor="w", padx=20, pady=(20, 6))

        tk.Button(
            painel, text="  Abrir Imagem",
            command=self.acao_abrir_imagem,
            bg=ACCENT, fg="#ffffff",
            font=("Segoe UI", 10, "bold"),
            activebackground="#6366f1", activeforeground="#ffffff",
            relief="flat", cursor="hand2", pady=10, anchor="w", padx=14
        ).pack(fill="x", padx=16)

        tk.Frame(painel, bg=BORDER, height=1).pack(fill="x", padx=16, pady=20)

        tk.Label(painel, text="FILTRO", font=("Segoe UI", 8, "bold"),
                 fg=MUTED, bg=PANEL).pack(anchor="w", padx=20, pady=(0, 8))
        ...
        # --- TEMPORÁRIO: controle de amplificação para gerar imagens da documentação ---
        tk.Label(painel, text="AMPLIFICAÇÃO", font=("Segoe UI", 8, "bold"),
                 fg=MUTED, bg=PANEL).pack(anchor="w", padx=20, pady=(0, 4))

        self._label_fator = tk.Label(painel, text="Fator: 4.0", font=("Segoe UI", 9),
                                     fg=TEXT, bg=PANEL)
        self._label_fator.pack(anchor="w", padx=20)

        tk.Scale(
            painel, variable=self.fator_amplificacao,
        ...
        tk.Button(
            painel, text="  Salvar Resultado",
            command=self.acao_salvar_resultado,
            bg=SUCCESS, fg="#0a0a14",
            font=("Segoe UI", 10, "bold"),
            activebackground="#10b981", activeforeground="#0a0a14",
            relief="flat", cursor="hand2", pady=10, anchor="w", padx=14
        ).pack(fill="x", padx=16)
    ...
    def _criar_botao_filtro(self, painel, nome):
        ...
        label = tk.Label(frame, text=nome, font=("Segoe UI", 10),
                         fg=TEXT, bg=PANEL, pady=9, padx=12, anchor="w")
    ...
    def _criar_area_imagens(self, painel):
        ...
        tk.Label(titulos, text="ORIGINAL", font=("Segoe UI", 8, "bold"),
                 fg=MUTED, bg=BG).pack(side="left", expand=True)
        tk.Label(titulos, text="RESULTADO", font=("Segoe UI", 8, "bold"),
                 fg=MUTED, bg=BG).pack(side="left", expand=True)

        frame_canvas = tk.Frame(painel, bg=BG)
        frame_canvas.pack(fill="both", expand=True)

        self.canvas_original = tk.Canvas(frame_canvas, bg=CARD,
                                         highlightthickness=1, highlightbackground=BORDER)
        self.canvas_original.pack(side="left", fill="both", expand=True, padx=(0, 8))

        self.canvas_resultado = tk.Canvas(frame_canvas, bg=CARD,
                                          highlightthickness=1, highlightbackground=BORDER)
    ...
    def _criar_barra_status(self):
        ...
        tk.Label(barra, textvariable=self.texto_status,
                 font=("Segoe UI", 9), fg=MUTED, bg=PANEL, anchor="w"
    ...
    def acao_abrir_imagem(self):
        ...
        if img is None:
            messagebox.showerror("Erro", "Não foi possível abrir essa imagem.")
            return
    ...
    def acao_aplicar_filtro(self):
        ...
        self.imagem_resultado = aplicar_filtro(
            self.imagem_original, self.filtro_selecionado.get(),
    ...
    def _exibir_no_canvas(self, canvas, imagem, colorida):
        ...
        imagem_redim = cv2.resize(imagem, (int(iw * escala), int(ih * escala)),
    ...
    def _mensagem_canvas(self, canvas, mensagem):
        ...
        canvas.delete("all")
        canvas.create_text(w // 2, h // 2, text=mensagem,
